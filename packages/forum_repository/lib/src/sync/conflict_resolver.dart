...
class ConflictResolver {
    ...
    for (final serverPost in serverPosts) {
      ...
      if (localByServerId != null) {
        ...
        if (localByServerId.localId != serverPost.localId) {
          ...
          print('DEBUG: Healing Post ID Drift: ${localByServerId.localId} -> ${serverPost.localId}');
        ...
        final localByUuid = await _database.getPostByLocalId(serverPost.localId);
        if (localByUuid != null) {
  ...
  Future<void> mergeServerComments(List<ForumComment> serverComments) async {
    for (final serverComment in serverComments) {
      // 1. Precise match by Server ID
      final localByServerId = await _database.getCommentByServerId(serverComment.id);

      if (localByServerId != null) {
        // MATCH BY SERVER ID
        if (localByServerId.localId != serverComment.localId) {
          // HEAL: UUID MISMATCH - The server updated its client_id (likely via backfill)
          print('DEBUG: Healing Comment ID Drift: ${localByServerId.localId} -> ${serverComment.localId}');
        ...
        final localByUuid = await _database.getCommentByLocalId(serverComment.localId);
        if (localByUuid != null) {
          // Sync success
  ...
  Future<void> _updateServerComment(ForumComment comment, String localId) async {
