...
class ForumDatabase extends _$ForumDatabase {
  ...
  Future<ForumPostData?> getPostByLocalId(String localId) async {
    return (select(
      forumPosts,
    )..where((post) => post.localId.equals(localId))).getSingleOrNull();
  }
  ...
  Future<ForumPostData?> getPostByServerId(String serverId) async {
    return (select(
      forumPosts,
    )..where((post) => post.serverId.equals(serverId))).getSingleOrNull();
  }
  ...
  Future<int> deletePost(String localId) async {
    return (delete(
      forumPosts,
    )..where((post) => post.localId.equals(localId))).go();
  }
  ...
  Future<int> updatePostLike({
    ...
    return (update(forumPosts)..where(
          (post) => post.serverId.equals(postId) | post.localId.equals(postId),
        ))
        .write(
          ForumPostsCompanion(
  ...
  Future<List<ForumCommentData>> getCommentsForPost(String postId) async {
    return (select(forumComments)
          ..where(
            (comment) =>
                comment.postId.equals(postId) & comment.isDeleted.equals(false),
          )
          ..orderBy([
  ...
  Future<ForumCommentData?> getCommentByServerId(String serverId) async {
    return (select(
      forumComments,
    )..where((c) => c.serverId.equals(serverId))).getSingleOrNull();
  }
  ...
  Future<int> deleteComment(String localId) async {
    return (delete(
      forumComments,
    )..where((c) => c.localId.equals(localId))).go();
  }
  ...
  Future<int> updateChildParentIds({
    ...
    return (update(
      forumComments,
    )..where((c) => c.parentCommentId.equals(oldParentLocalId))).write(
      ForumCommentsCompanion(
  ...
  Future<int> updateSyncQueueStatus({
    ...
    return (update(syncQueue)..where((item) => item.id.equals(id))).write(
      SyncQueueCompanion(
        status: Value(status),
        ...
        retryCount: Value(
          (await (select(
                syncQueue,
              )..where((item) => item.id.equals(id))).getSingle()).retryCount +
              1,
        ),
  ...
  Future<void> batchInsertComments(
    List<ForumCommentsCompanion> comments,
  ) async {
    await batch((batch) {
      batch.insertAll(
        forumComments,
  ...
  Future<void> batchInsertLineComments(
    List<ForumLineCommentsCompanion> comments,
  ) async {
    await batch((batch) {
      batch.insertAll(
        forumLineComments,
  ...
  Future<void> incrementLineCommentCount(String lineId) async {
    final line = await (select(
      forumAnswerLines,
    )..where((l) => l.lineId.equals(lineId))).getSingleOrNull();
    if (line != null) {
      await (update(
        forumAnswerLines,
      )..where((l) => l.lineId.equals(lineId))).write(
